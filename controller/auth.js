...
const { User, validate, validateResetPassword, validateLogin, validateOTP, varifyValidate } = require('../model/User');
...
router.post('/login', async(req, res ) => {
    ...
    let userObject =  await helper.varifyCredentials((req.body.email.trim()).toLowerCase(), req.body.pin_code)
    if(userObject){
...
router.post('/resetPassword', async(req, res) => {
    ...
    let status = await helper.varifyPasswordAndUpdate(req.body.user_id , req.body.old_pin_code , req.body.new_pin_code)
    if(status == true){
        let response = {
...
router.post('/tokenSwitch', async(req, res) => {
    ...
    if(req.body.user_id && req.body.symbol){
        ...
        let response = await helper.updateTheRecord(req.body.user_id, req.body.symbol, req.body.status)
        res.status(response.status).send(response);
    }else{
        let response = {
            message  :   'payload missing!!!'
