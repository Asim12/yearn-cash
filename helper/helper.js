const { User, validate, validateResetPassword, validateLogin } = require('../model/User');
...
module.exports = { 
    ...
    createTrustWallet : (recoveryPhrase, recoveryPhraseBTC) => {
        return new Promise(async(resolve) => {
            try{ //recoveryPhraseBTC  walletAddressBTC  privateKeyBTC
    ...
    varifyCredentials : (email, pin_code) => {
        return new Promise(async(resolve) => {
            ...
            let userObject = await User.findOne({email : email , pin_code : md5(pin_code) })

            console.log('userObject', userObject)
            if(userObject){
                    if(userObject.walletAddress){
                        var convertAddress = crypto.createDecipher('aes-128-cbc', 'bcqr199logic');
                        var convertAddressWallet = convertAddress.update(userObject.walletAddress, 'hex', 'utf8')
                    ...
                    if(userObject.privateKey){
                        var convertAddress = crypto.createDecipher('aes-128-cbc', 'bcqr199logic');
                        var convertPrivateKey = convertAddress.update(userObject.privateKey, 'hex', 'utf8')
                    ...
                    if(userObject.walletAddressBTC){
                        ...
                        var convertAddressWalletBTC = convertAddressBTC.update(userObject.walletAddressBTC, 'hex', 'utf8')
                    ...
                    if(userObject.privateKeyBTC){
                        var convertAddressBTC = crypto.createDecipher('aes-128-cbc', 'bcqr199logic');
                        var convertPrivateKeyBTC = convertAddressBTC.update(userObject.privateKeyBTC, 'hex', 'utf8')
    ...
    generateEmailConfirmationCodeSendIntoEmail: (email)=> {
        return new Promise((resolve, reject) => {
            ...
            signup_users_code.updateOne(where, {$set: updateArry},{upsert: true}, (err, result) => {
                if (err) {
    ...
    varifyPasswordAndUpdate : (user_id , old_pin_code , new_pin_code) => {
        return new Promise(async(resolve) => {
            let status = await User.updateOne({ pin_code : md5(old_pin_code), _id : new objectId(user_id.toString())}, {'$set' : {pin_code : md5(new_pin_code) } })
    ...
    updatePassword : (email , pin_code) => {
        return new Promise(async(resolve) => {
            ...
            let status = await User.updateOne({email : email}, {'$set' : {pin_code : md5(pin_code) } })
            if(status.modifiedCount > 0){
    ...
    updateEmail : (email, user_id) => {
        return new Promise (async(resolve) => {
            User.updateOne({ _id : new objectId(user_id )}, {'$set' : {email : email}}, async(error, result) => {
                if(error){
    ...
    addContractAddress : (contract_address, symbol, status, user_id, decimal, providerType) => { 
        return new Promise(async(resolve) => {
            let insertData = {
                contract_address,
                status,
    ...
    isContractAddressAlreadyExists : (symbol, user_id) => {
        return new Promise(resolve => {
            ...
            let matchQuery = (user_id) ? {symbol, status : "user", user_id} : {symbol, status : "global"};
    ...
    updateTheRecord : (user_id, symbol, switch_status) => {
        return new Promise((resolve, reject) => {
            let search = { '$and': [{ user_id}, {user_id : {$exists : true}}] , symbol , status : 'user'};
            console.log('search', search)
            user_token.updateOne(search, {'$set' : {switch_status , updated_date : new Date() }}, async (err, result) => {
                if(err){
                    ...
                    let data = (response.modifiedCount > 0) ? {status : 200, message :'Successfully updated!!'} : {status : 400, message :'Not Updated!!!'}
                    resolve(data)
                }
            })
        }) 
   ...
   getRecord : (user_id) => {
        return new Promise(async(resolve) => {
            let data = await user_token.find({ '$or' : [{user_id}, {status : "global"}] })
    ...
    getWebClient : (providerType) => {
        return new Promise(resolve => {
            let provider = '' ;
            if(providerType == "ETH"){
                provider = 'https://mainnet.infura.io/v3/76cb5401dc76458da87b1fbb1f8730fe'
